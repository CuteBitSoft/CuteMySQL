...
public:
     ...
     */
    QSqlException(const char* aErrorMessage, int ret);

    QSqlException(const std::string& aErrorMessage, int ret) :
     ...
     * @param[in] aErrorMessage The string message describing the SQLite error
     */
    explicit QSqlException(const char* aErrorMessage) :
        QSqlException(aErrorMessage, -1) // 0 would be SQLITE_OK, which doesn't make sense
    {
    }
    explicit QSqlException(const std::string& aErrorMessage) :
