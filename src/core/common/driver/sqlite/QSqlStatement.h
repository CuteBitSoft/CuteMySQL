...
public:
	/**
		...
		*/
	QSqlStatement(const QSqlDatabase* aDatabase, const char* apQuery);
	...
	SQLITECPP_PURE_FUNC
		int getIndex(const char * apName) const;
	...
	/**
		* @brief Bind a 64bits int value to a named parameter "?NNN", ":VVV", "@VVV" or "$VVV" in the SQL prepared statement (aIndex >= 1)
		*/
	void bind(const char* apName, const uint64_t         aValue)
	{
		bind(getIndex(apName), aValue);
	}
	...
	/**
		* @brief Bind a 64bits int value to a named parameter "?NNN", ":VVV", "@VVV" or "$VVV" in the SQL prepared statement (aIndex >= 1)
		*/
	void bind(const std::string& aName, const uint64_t         aValue)
	{
		bind(aName.c_str(), aValue);
	}
	...
	/**
		...
		*/
	QSqlColumn  getColumn(const char* apName) const;
...
private:
	...
	void checkRow() const
	{
		if (false == mbHasRow)
		{
			throw SQLite::QSqlException("No row to get a column from. executeStep() was not called, or returned false.");
		}
	}
	...
	void checkIndex(const int aIndex) const
	{
		...
		{
			throw SQLite::QSqlException("Column index out of range.");
		}
	}
